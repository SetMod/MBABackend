...
class MBAnalyze:
    ...
    def preprocess(self, df: DataFrame, save_csv: bool = False):
        ...
        if save_csv:
            df.to_csv(f"{self.preprocess_data_path}/pd_{self.id}.csv", index=False)
    ...
    def transform(self, df: DataFrame, save_csv: bool = False):
        ...
        if save_csv:
            df_set.to_csv(
                f"{self.data_transformation_path}/td_{self.id}.csv", index=False
            )
    ...
    def analyze_preprocess_data(self, df: DataFrame):
        ...
        transactions_cost_item: pd.Series = (
    ...
    def create_association_rules(
        self, frequent_itemsets: DataFrame, save_csv: bool = False
    ):
        ...
        if save_csv:
            asoc_rules.to_csv(
                f"{self.preprocess_data_path}/pd_{self.id}.csv", index=False
            )
