...
class OrganizationRolesService:
    ...
    def get_all_organization_roles(self, dump: bool = True):
        ...
        if len(organization_roles) > 0:
            return (
                self.organization_roles_schema.dump(organization_roles, many=True)
                if dump
    ...
    def get_organization_role_by_id(self, id: int, dump: bool = True):
        organization_role = (
            db.session.query(OrganizationRoles)
            .where(OrganizationRoles.id == id)
            .first()
        )
    ...
    def get_organization_role_by_name(
        self, name: str, dump: bool = True
    ) -> OrganizationRoles:
        organization_role = (
            db.session.query(OrganizationRoles)
            .where(OrganizationRoles.name == name)
            .first()
        )
    ...
    def update_organization_role(
        self, id, updated_organization_role: OrganizationRoles, dump: bool = True
    ):
        organization_role = self.get_organization_role_by_id(id, dump=False)

        try:
            if type(organization_role) is OrganizationRoles:
                organization_role.name = updated_organization_role.name
                organization_role.description = updated_organization_role.description
                db.session.commit()
                return (
                    self.organization_roles_schema.dump(organization_role)
                    if dump
    ...
    def delete_organization_role(self, id: int, dump: bool = True):
        organization_role = self.get_organization_role_by_id(id, dump=False)
    ...
    def map_organization_role(self, organization_role_dict: dict):
        try:
            ...
            name = organization_role_dict["name"]
            description = organization_role_dict["description"]
            organization_role = OrganizationRoles(name=name, description=description)
